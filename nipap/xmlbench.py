...
class Request:
    ...
    def makeRequest(self):
        ...
        proxy.callRemote(self.method, *self.params).addCallbacks(
            self.retSuccess, self.retFail
        )
...
class Benchmark:
    def __init__(
        ...
        concurrent=10,
        total=100,
        url='http://localhost:7080/XMLRPC',
        method='date',
        params=None,
    ):
        if params is None:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-p', '--port', dest='port', type='int', default='1337', help="TCP port"
    )
