...
def list_pool():
    ...
    try:
        ...
        attr = extract_pool_attr(request.json)
    except ValueError:
        attr = {}
...
def list_prefix():
    ...
    # fetch attributes from request.json
    attr = extract_prefix_attr(request.json)
...
def add_prefix():
    ...
    if 'vrf' in request.json:
        try:
            ...
            return json.dumps({'error': 1,
                               'message': "Invalid VRF ID '%s'" % request.json['vrf']})
        except NipapError as e:
            return json.dumps({'error': 1,
                               'message': e.args,
                               'type': type(e).__name__})
...
def add_current_vrf():
    ...
    if vrf_id is not None:
        ...
        session['current_vrfs'][vrf_id] = {'id': vrf.id,
                                           'rt': vrf.rt,
                                           'name': vrf.name,
...
def get_current_vrfs():
    ...
    if len(cur_vrfs) > 0:
        ...
        for vrf in res['result']:
            session['current_vrfs'][vrf.id] = {'id': vrf.id,
                                               'rt': vrf.rt,
                                               'name': vrf.name,
                                               'description': vrf.description}
