...
nipapAppControllers.controller('VRFListController', function ($scope, $http) {
    if ($scope.query === '' || $scope.query === undefined) {
        ...
        $http.post('/xhr/list_vrf',
            ...
            .then(function (response) {
                if (response.data.hasOwnProperty('error')) {
                    ...
                    $scope.loaded = allVrfs.slice(0, VRF_BATCH_SIZE);
                    $scope.vrfs = $scope.loaded;
                }
            })
        ...
        // Dynamically adding values to $scope.vrfs
        $scope.performVrfNextPage = function () {
            if (typeof $scope.vrfs === "undefined") {
                return;
            }

            if (!$scope.query === '' || $scope.vrfs.length === allVrfs) {
                return;
            }
