...
class AuthController(BaseController):
    ...
    def login(self):
        ...
        try:
            ...
            auth = auth_fact.get_auth(user, pw, 'nipap')
            if not auth.authenticate():
                c.error = 'Invalid username or password'
                return render('login.html')
        except AuthError as exc:
            c.error = 'Authentication error'
            if pw == pw.encode("ascii", "ignore"):  # non-ascii?
                log.info('Authentication error: password for %s has non-ascii '
                         'characters', user)
            else:
                log.exception('Authentication error for ' + user, exc)
                return render('login.html')
            ...
            if session.get('path_before_login'):
                ...
                # if previous target is unknown just send the user to a welcome
                # page
