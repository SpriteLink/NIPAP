...
class NipapRestTest(unittest.TestCase):
    ...
    def test_prefix_add(self):
        ...
        text = request.text
        self.assertRegexpMatches(text,"specify 'prefix' or 'from-prefix' or 'from-pool'")

        attr['attr']['prefix'] = '1.3.3.0/24'
        request = requests.post(self.server_url, headers=self.headers, json = attr)
        text = request.text
        self.assertRegexpMatches(text, "Either description or node must be specified.")

        attr['attr']['description'] = 'test prefix'
        request = requests.post(self.server_url, headers=self.headers, json = attr)
        text = request.text
        self.assertRegexpMatches(text, "Unknown prefix type")

        attr['attr']['type'] = 'assignment'
        attr['attr']['order_id'] = 'test'
        ...
        attr['attr']['id'] = result['id']
        self.assertGreater(attr['attr']['id'], 0)
        ...
        #print(list_prefix)
        #list_prefix = dict([(str(k), str(v)) for k, v in list_prefix.items()])
        ...
        attr = {'attr':{}}
        attr['attr']['description'] = 'test for from-prefix 1.3.3.0/24'
        attr['attr']['type'] = 'host'
        attr['attr']['order_id'] = 'test'
        ...
        expected_host.update(attr['attr'])
        print(expected_host)
    ...
    def test_prefix_remove(self):
        ...
        # add 1.3.3.4/24
        attr = {'attr':{}}
        attr['attr']['prefix'] = '1.3.4.0/24'
        attr['attr']['description'] = 'test delete prefix'
        attr['attr']['type'] = 'assignment'
        attr['attr']['order_id'] = 'test'
